...
class OpportunistcApp(Application):
    ...
    def install(self, node: QNode, simulator: Simulator):
        ...
        self.memory: QuantumMemory = self.own.memories[
            0
        ]  # インストール先ノードのメモリ
    ...
    def est_init(self, idx: int):
        ...
        log.debug(
            f"[est_init] {self.own} req {idx} {self.net.requests[idx]} lost established link"
        )
    ...
    def get_route_nodes(self, src: QNode, dest: QNode) -> List[QNode]:
        ...
        try:
            ...
            log.debug(
                f"[get_route_nodes] {self.own} no route found: src={src}, dest={dest}"
            )
    ...
    def reserve_nopp(self, src: QNode, dest: QNode, idx: int):
        ...
        for i in range(len(route_nodes) - 1):
            qc = self.get_qc(route_nodes[i], route_nodes[i + 1])
            if qc.is_entangled and (
                not qc.is_reserved or qc.reservation == idx
            ):  # 有効かつ(予約されていないor自分が予約している)
                pass
            else:
                isallready = False
                break
    ...
    def nopp_swapping(self, src: QNode, dest: QNode, idx: int):
        ...
        if random.random() < self.p_swap:  # swap成功
            ...
            if not next_hop == dest:
                # continue swapping
                log.debug(
                    f"[nopp_swapping] {self.own} req {idx} {self.net.requests[idx]} success swapping {self.own} <-> {next_hop}"
                )
            else:
                ...
                log.debug(
                    f"[nopp_swapping]{self.own} req {idx} {self.net.requests[idx]} completed! {self.net.established[idx]}"
                )
        else:  # swap失敗
            log.debug(
                f"[nopp_swapping] req {idx} {self.net.requests[idx]} failed to swap {route_nodes[pos]} <-> {route_nodes[pos + 1]}"
            )
            self.est_init(idx)
            self.releaselinks(route_nodes=route_nodes)
    ...
    def opp_swapping(self, src: QNode, dest: QNode, idx: int):
        ...
        qc = self.get_qc(tmpsrc, next_hop)
        log.debug(
            f"[opp_swapping]{idx}, {qc}, {qc.is_entangled}, {qc.is_reserved}, {qc.reservation}"
        )
        qc.is_entangled = False
        ...
        if random.random() < self.p_swap:  # swap成功
            log.debug(
                f"[opp_swapping]{self.own} req {idx} {self.net.requests[idx]} swapping success {self.own} <-> {next_hop}"
            )
            self.net.requests[idx].pos += 1
            self.net.established[idx][1] = next_hop
            self.net.established[idx][2] = t
            ...
            if not next_hop == dest:
                # continue swapping
                log.debug(
                    f"[opp_swapping]{self.own} req {idx} {self.net.requests[idx]} new! established link {self.net.established[idx]}"
                )
            else:
                ...
                self.net.requests[idx].waitingtime = tc.sec
                log.debug(
                    f"[opp_swapping]{self.own} req {idx} {self.net.requests[idx]} completed! {self.net.established[idx]}"
                )
        else:  # swap失敗
            log.debug(
                f"[opp_swapping] req {idx} failed to swap {route_nodes[0]} <-> {route_nodes[1]}"
            )
            self.est_init(idx)
    ...
    def eg_routine(self):
        ...
        for qc in self.adj_qc:
            if qc.is_entangled and tc.sec - qc.born.sec > self.lifetime - self.alpha:
                qc.is_entangled = False
                log.debug(
                    f"[eg_routine]{qc}: born {qc.born.sec}s, now {tc.sec}s --->decoherenced"
                )
        ...
        for qc in self.adj_qc:
            if not qc.is_entangled:
                ...
                if random.random() < self.p_gen:
                    ...
                    # #nexthop設定
                    if (
                        qc.node_list[0] == self.own
                    ):  # 1つのqcに対して二重にチャレンジを行うのを防ぐ
