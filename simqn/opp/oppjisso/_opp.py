...
class OpportunistcApp(Application):
    ...
    # def swapping(self, src: QNode, dest: QNode, idx: int):
    #     tc = self._simulator.tc
    #     if random.random() < self.p_swap:
    #         self.net.established[idx][1] = dest
...
def exp2(islog: bool = False, isopp: bool = False,
         p_gen: float = p_gen, p_swap: float = p_swap,
         L: float = lifetime, N: int = request_number, seed: int = None):
    s = Simulator(0, 100000, accuracy=1)
    if islog:
        log.logger.setLevel(logging.DEBUG)
        log.install(s)

    topo1 = LineTopology(nodes_number=nodes_number, nodes_apps=[OpportunistcApp(p_gen=p_gen, p_swap=p_swap, lifetime=L, isopp=isopp)],
...
def exp3(islog: bool = False, isopp: bool = False,
    ...
    if islog:
        ...
        log.install(s)
    drop_rate = 0.0
    topo2 = GridTopology(nodes_number=M**2, nodes_apps=[OpportunistcApp(p_gen=p_gen, p_swap=p_swap, lifetime=L, isopp=isopp, alpha=alpha)],
                        qchannel_args={"drop_rate": drop_rate, "delay": qchannel_delay},
                        cchannel_args={"delay": cchannel_delay},
                        memory_args=[{"capacity": memory_capacity}])
...
def exp_linetopo(islog: bool = False, isopp: bool = False,
         p_gen: float = p_gen, p_swap: float = p_swap,
         L: float = lifetime, N: int = request_number, M: int = size_of_grid, seed: int = None,
         alpha: float = 0):
    s = Simulator(0, 1000000, accuracy=1)
    ...
    nodes_number = 7
    topo = LineTopology(nodes_number=nodes_number, nodes_apps=[OpportunistcApp(p_gen=p_gen, p_swap=p_swap, lifetime=L, isopp=isopp, alpha=alpha)],
                        qchannel_args={"drop_rate": drop_rate, "delay": qchannel_delay},
                        cchannel_args={"delay": cchannel_delay},
                        memory_args=[{"capacity": memory_capacity}])
...
def exp5():
    ...
    for i in range(40):
        waitingtime_nopp = exp_linetopo(isopp=False, p_gen=p, p_swap=p_swap, L=L, N=N, M=M)
        totalwaitings_nopp.append(waitingtime_nopp)
        waitingtime_opp = exp_linetopo(isopp=True, p_gen=p, p_swap=p_swap, L=L, N=N, M=M)
        totalwaitings_opp.append(waitingtime_opp)
        print(p, i, 'res', waitingtime_nopp, waitingtime_opp)
    avarage_waitingtime_nopp = sum(totalwaitings_nopp) / len(totalwaitings_nopp)
    avarage_waitingtime_opp = sum(totalwaitings_opp) / len(totalwaitings_opp)
    avarage_nopp.append(avarage_waitingtime_nopp)
    avarage_opp.append(avarage_waitingtime_opp)
...
def exp6():
    ...
    for a in range(10):
        ...
        totalwaitings_opp = []
        for i in range(40):
            waitingtime_nopp = exp_linetopo(isopp=False, p_gen=p, p_swap=p_swap, L=L, N=N, M=M, alpha=a)
            totalwaitings_nopp.append(waitingtime_nopp)
            waitingtime_opp = exp_linetopo(isopp=True, p_gen=p, p_swap=p_swap, L=L, N=N, M=M, alpha=a)
            totalwaitings_opp.append(waitingtime_opp)
            print(a, i, 'res', waitingtime_nopp, waitingtime_opp)
        avarage_waitingtime_nopp = sum(totalwaitings_nopp) / len(totalwaitings_nopp)
        avarage_waitingtime_opp = sum(totalwaitings_opp) / len(totalwaitings_opp)
        avarage_nopp.append(avarage_waitingtime_nopp)
        avarage_opp.append(avarage_waitingtime_opp)
...
def exp7():
    ...
    for a in range(10):
        ...
        for p in p_gens:
            average_pp = []
            for i in range(40):
                waitingtime = exp3(isopp=True, p_gen=p, p_swap=p_swap, L=L, N=N, M=M, seed = 1, alpha=a)
                average_pp.append(waitingtime)
                print(a, p, i, 'res', waitingtime)
            #avarage_waitingtime_nopp = sum(totalwaitings_nopp) / len(totalwaitings_nopp)
            average_p = sum(average_pp) / len(average_pp)
            average_a.append(average_p)
        average.append(average_a)
